...
namespace ntx20.command.ws
{
    ...
    {
        private static readonly ILogger _logger = Logging.LoggerFactory.CreateLogger("ntx20.command.ws");
            ...
            if (options.TheService == null)
            {
                ...
                command.OnExecute(() =>
                {
                    ...
                    if (knowAppTypes[appType] == "ntx20")
                    {
                        ...
                        var meta = ntx20.command.Authentication.CreateMetadata(uri, options.WsUsernameEnvOption, options.WsPasswordEnvOption, headersOption.Values);
